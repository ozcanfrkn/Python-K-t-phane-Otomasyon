...
class yeniKitap(QWidget):
    ...
    def kaydet(self):
        ...
        baglac.execute("INSERT INTO kitaplar (kitap_ad) VALUES (?)",(isim,))
        baglanti.commit()
...
class yeniOgrenci(QWidget):
    ...
    def kaydet(self):
        ...
        baglac.execute("INSERT INTO ogrenciler (ogrenci_ad) VALUES (?)",(isim,))
        baglanti.commit()
...
class yeniOdunc(QWidget):
    ...
    def kaydet(self):
        ...
        baglac.execute("INSERT INTO odunc (ogrenci_ad,kitap_ad) VALUES (?,?)",(ogrenciismi,kitapismi))
        baglanti.commit()
...
class yeniiade(QWidget):
    ...
    def kaydet(self):
        ...
        ogrenciismi = self.ogrenciismi.text()
        kitapismi = self.kitapismi.text()
        baglac.execute("DELETE FROM odunc WHERE ogrenci_ad = ? AND kitap_ad = ?",(ogrenciismi,kitapismi))
        baglanti.commit()
...
class kitapListesi(QWidget):
    def __init__(self):
        ...
        kitaplar = baglac.execute("SELECT * FROM kitaplar")
    ...
    def kitapBilgi(self,item):
        ...
        kitapismi = item.text()
        kontrol = baglac.execute("SELECT * FROM kitaplar WHERE kitap_ad = ?",(kitapismi,))
        durum = kontrol.fetchall()[0][2]

        if(durum==0):
            QMessageBox.information(self,"Kitap Bilgisi",kitapismi + " isimli kitap şu anda boşta !")
        else:
            kimde = baglac.execute("SELECT * FROM odunc WHERE kitap_ad = ?",(kitapismi,))
...
class ogrenciListesi(QWidget):
    def __init__(self):
        ...
        ogrenciler = baglac.execute("SELECT * FROM ogrenciler")
    ...
    def ogrenciBilgi(self, item):
        ...
        kontrol = baglac.execute("SELECT * FROM odunc WHERE ogrenci_ad = ?", (ogrenciismi,))
        ...
        if (say == 0):
            ...
            hangi = baglac.execute("SELECT * FROM odunc WHERE ogrenci_ad = ?", (ogrenciismi,))
...
class oduncListesi(QWidget):
    def __init__(self):
        ...
        oduncler = baglac.execute("SELECT * FROM odunc")
...
class yardimHakkimizda(QWidget):
    def __init__(self):
        ...
        baslik.setFont(baslikfont)
        yazi = QLabel("http://www.alparslan.edu.tr/")
